...
@echo Checking for binaries in %OPENSSL_PATH%/out32dll.dbg
@if exist %OPENSSL_PATH%/out32dll.dbg/libeay32.dll (
...
@if exist %OPENSSL_PATH%/out32dll.dbg/libeay32.lib  (
...
@if exist %OPENSSL_PATH%/out32dll.dbg/ssleay32.dll (
...
@if exist %OPENSSL_PATH%/out32dll.dbg/ssleay32.lib  (
...
@for %%i in (%OPENSSL_PATH%\out32dll.dbg\*.lib) do @copy /Y %%i %MICO_PATH%\win32-bin\lib 2>output
@for %%i in (%OPENSSL_PATH%\out32dll.dbg\*.dll) do @copy /Y %%i %MICO_PATH%\win32-bin 2>output
...
@echo  "$(OPENSSL_PATH)/out32dll.dbg/*.dll -> $(MICO_PATH)/win32-bin"
@echo  "$(OPENSSL_PATH)/out32dll.dbg/*.lib -> $(MICO_PATH)/win32-bin/lib"
@echo Syntax:
