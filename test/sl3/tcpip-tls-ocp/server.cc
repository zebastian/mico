...
main (int argc, char *argv[])
{
	...
	tcpip_builder->add_tcpip_acceptor_options(sseq, "localhost", 2745, 0, 0, FALSE);
	arg = tcpip_builder->reap_args();
	acquirer = curator->acquire_credentials(arg);
	creds = acquirer->get_credentials(TRUE);

	tcpip_builder = SL3TCPIP::TCPIPArgBuilder::_narrow
	    (tcpip_fact->create_arg_builder(SL3CM::CU_AcceptOnly));
	assert(!CORBA::is_nil(tcpip_builder));
	tcpip_builder->add_tcpip_acceptor_options(sseq, "localhost", 5678, 0, 0, FALSE);
	arg = tcpip_builder->reap_args();
	acquirer = curator->acquire_credentials(arg);
	creds = acquirer->get_credentials(TRUE);
	...
	tcpip_builder = SL3TCPIP::TCPIPArgBuilder::_narrow
	    (tcpip_fact->create_arg_builder(SL3CM::CU_AcceptOnly));
	assert(!CORBA::is_nil(tcpip_builder));
	tcpip_builder->add_tcpip_acceptor_options(sseq, "localhost", 8912, 0, 0, FALSE);
	arg = tcpip_builder->reap_args();
	acquirer = curator->acquire_credentials(arg);
	creds = acquirer->get_credentials(TRUE);
	...
	wcout << endl;
        ...
        ocp_list->length(2);
        ocp_list[0u] = creds_list[0u];
        ocp_list[1u] = creds_list[5u];
