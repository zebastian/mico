...
//  	// bind to ATLAS server - we use bind so we need to obtain
//  	// transport credentials at the first
//  	CORBA::Object_var tfobj = orb->resolve_initial_references("SL3TCPIP::ArgBuilderFactory");
//  	SL3AQArgs::ArgBuilderFactory_var tfact
//  	    = SL3AQArgs::ArgBuilderFactory::_narrow(tfobj);
//  	assert(!CORBA::is_nil(tfact));
//  	SL3TCPIP::TCPIPArgBuilder_var tsi_builder
//  	    = SL3TCPIP::TCPIPArgBuilder::_narrow
//  	    (tfact->create_arg_builder(SL3CM::CU_InitiateOnly));
//  	assert(!CORBA::is_nil(tsi_builder));
//  	SL3AQArgs::Argument_var tsi_args = tsi_builder->reap_args();
//  	tfobj = orb->resolve_initial_references
//  	    ("TransportSecurity::SecurityManager");
//  	TransportSecurity::SecurityManager_var tsi_secman
//  	    = TransportSecurity::SecurityManager::_narrow(tfobj);
//  	assert(!CORBA::is_nil(tsi_secman));
//  	TransportSecurity::CredentialsCurator_var tsi_curator
//  	    = tsi_secman->credentials_curator();
//  	TransportSecurity::CredentialsAcquirer_ptr tsi_acquirer
//  	    = tsi_curator->acquire_credentials(tsi_args);
//  	TransportSecurity::OwnCredentials_ptr tsi_creds = tsi_acquirer->get_credentials(TRUE);

//  	CORBA::Object_var atobj = orb->bind
//  	    ("IDL:omg.org/ATLAS/AuthTokenDispenser:1.0", "inet:localhost:3344");
//  	assert(!CORBA::is_nil(atobj));
//  	ATLAS::AuthTokenDispenser_var atlas
//  	    = ATLAS::AuthTokenDispenser::_narrow(atobj);
//  	assert(!CORBA::is_nil(atlas));
	...
	SL3Authorization::TokenProcessor_var tp
	    = new MICOSL3_AC::AttributeCertChainTokenProcessor("ca.pem");
