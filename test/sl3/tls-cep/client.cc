...
main (int argc, char *argv[])
{
  ...
  builder->add_tcpip_initiator_options("localhost", 6589, 6599, 0);
  ...
  SL3TLS::OpenSSLConfigArgBuilder_var builder2
      = SL3TLS::OpenSSLConfigArgBuilder::_narrow
      (fact->create_arg_builder(SL3CM::CU_InitiateOnly));
  assert(!CORBA::is_nil(builder2));
  builder2->add_tcpip_initiator_options("localhost", 12445, 12454, 0);
  builder2->add_tls_initiator_options("ca_cert.pem", "s_cert.pem", "s_key.pem");
  builder2->add_initiator_verify_depth(1);
