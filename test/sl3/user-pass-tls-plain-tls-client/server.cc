...
class IdentityProcessor_impl
    : virtual public SL3TLS::TLSX509IdentityProcessor,
      virtual public CORBA::LocalObject
{
public:
    virtual CORBA::Boolean
    verify_identity(SL3PM::Principal* ident);
};

CORBA::Boolean
IdentityProcessor_impl::verify_identity(SL3PM::Principal* ident)
{
    //cerr << "IdentityProcessor_impl::verify_identity" << endl;
    assert(ident != NULL);
    SL3PM::PrincipalName prname = ident->the_name();
    if (strcmp(prname.the_type.in(), SL3PM::NT_X509DirectoryNamePath) == 0
	&& prname.the_name.length() == 2) {
	wstring name = CORBA::wstring_dup(prname.the_name[0].in());
	wstring ca = CORBA::wstring_dup(prname.the_name[1].in());
	if (name == L"/CN=Karel Gardas/emailAddress=kgardas@objectsecurity.com"
	    && ca ==  L"/C=CZ/L=Roznov/O=SignEveryThing/CN=Sign Every Thing, s.r.o./emailAddress=set@iol.cz") {
	    return TRUE;
	}
    }
    return FALSE;
...
main (int argc, char *argv[])
{
	...
	csi_builder->add_identity_processor(new IdentityProcessor_impl);
