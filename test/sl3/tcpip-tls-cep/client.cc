...
main (int argc, char *argv[])
{
  ...
  builder->add_tcpip_initiator_options("localhost", 5789, 5799, 0);
  ...
  SL3AQArgs::ArgBuilderFactory_var fact2
      = SL3AQArgs::ArgBuilderFactory::_narrow(obj);
  assert(!CORBA::is_nil(fact2));
  SL3TCPIP::TCPIPArgBuilder_var builder2
      = SL3TCPIP::TCPIPArgBuilder::_narrow
      (fact2->create_arg_builder(SL3CM::CU_InitiateOnly));
  assert(!CORBA::is_nil(builder2));
  builder2->add_tcpip_initiator_options("localhost", 11345, 11354, 0);
  ...
  hello->hello ();
  hello2->hello();
  TransportSecurity::TargetCredentials_var tcreds3 = secman->get_target_credentials(hello);
  orb->destroy();
  return 0;
}
